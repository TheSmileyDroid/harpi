...
- **V3 Brackets mark operation.** Corner brackets sit only on the element under operation right now: the playing panel (`.is-playing`) and focus states. Anywhere else, including as decoration on static panels, is a fail. Nav items never get brackets; the current one brightens with a 1px underline.
