...
class SoundboardSourceManager:
    ...
    def __init__(self, api: HarpiAPI, guild_id: int) -> None:
        self.api = api
    ...
    async def start_source(
        ...
        if node_id in self._node_to_layer:
            await self.stop_source(node_id)
        ...
        if volume != 100:
            await self.set_volume(node_id, volume)

        logger.info(f"Started source {node_id} -> layer {layer_id}")
        return layer_id

    async def stop_source(self, node_id: str) -> None:
        ...
        try:
            await self.api.remove_background_audio(self.guild_id, layer_id)
    ...
    async def set_volume(self, node_id: str, volume: float) -> None:
        ...
        try:
            await self.api.set_background_volume(
    ...
    async def stop_all(self) -> None:
        ...
        try:
            await self.api.clean_background_audios(self.guild_id)
        except Exception as e:
