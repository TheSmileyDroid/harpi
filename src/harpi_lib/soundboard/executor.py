...
from src.harpi_lib.musicdata.ytmusicdata import YoutubeDLSource
...
class SoundboardExecutor:
    ...
    def __init__(self, api: HarpiAPI) -> None:
        self.api = api
    ...
    async def _create_audio_source(
        ...
        Note: This method calls YoutubeDLSource.from_url() which does not
        exist on that class. This code path appears to be unused/broken.
        """
        try:
            ...
            logger.opt(exception=True).error(
                f"Failed to create audio source from {url}: {e}"
            )
            return None
    ...
    async def stop(self, guild_id: int) -> None:
        ...
        try:
            await self.api.clean_background_audios(guild_id)
        except Exception as e:
