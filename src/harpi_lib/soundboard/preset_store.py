...
Thread safety
-------------
``PresetStore`` performs synchronous file I/O (reads and writes JSON).
All calls originate from Quart's single-threaded async event loop, so
there is no concurrent access.  If this code is ever called from multiple
threads, file-level locking (e.g. ``fcntl.flock``) would be needed.
This is an **accepted risk** (LOW severity).
"""
...
class PresetStore:
    ...
    def _load_guild_presets(
        ...
        try:
            ...
            logger.opt(exception=True).error(
                f"Failed to load presets for guild {guild_id}: {e}"
            )
            return {}
    ...
    def _save_guild_presets(
        ...
        try:
            ...
            logger.opt(exception=True).error(
                f"Failed to save presets for guild {guild_id}: {e}"
            )
            raise
