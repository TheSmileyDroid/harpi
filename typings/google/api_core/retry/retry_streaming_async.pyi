...
if TYPE_CHECKING:
    _P = ParamSpec("_P")
    _Y = TypeVar("_Y")
async def retry_target_stream(target: Callable[_P, AsyncIterable[_Y] | Awaitable[AsyncIterable[_Y]]], predicate: Callable[[Exception], bool], sleep_generator: Iterable[float], timeout: float | None = ..., on_error: Callable[[Exception], None] | None = ..., exception_factory: Callable[[list[Exception], RetryFailureReason, float | None], tuple[Exception, Exception | None],] = ..., init_args: tuple = ..., init_kwargs: dict = ..., **kwargs) -> AsyncGenerator[_Y, None]:
